/* global confirm, console, kintone, location, setTimeout */

/**
 * kSQL-FlowNet の実行管理・監査履歴アプリを新規作成する（ブラウザ Console 用）
 *
 * 使い方:
 ...
 *
 * 運用ガイダンス:
 *   - APIトークンは実行管理・監査履歴ともに「レコード追加・レコード閲覧・レコード編集」で発行する。
 *     本番運用では削除権限は不要（ロック解放はキークリア/tombstone UPDATE方式）。
 *     試験清掃で削除が必要な場合は、本番トークンと分離した別トークンを使用する。
 *   - アプリのアクセス権はアプリ管理者とサービスアカウントに限定し、
 *     一般ユーザーには閲覧権限のみを付与することを推奨する。
 *   - kSQL-Flow 所有のJOBログアプリは本テンプレートの対象外。
 *     attempt_id等の相関フィールドはkSQL-Flow M1で追加済み。
 *
 * 安全性:
...
(async () => {
  ...
  const APP_DEFINITIONS = [
    {
      name: "kSQL-FlowNet 実行管理",
      appIdEnvironmentVariable: "KSQL_FLOWNET_STATE_APP_ID",
      tokenEnvironmentVariable: "KSQL_FLOWNET_STATE_API_TOKEN",
      fields: createExecutionFields(),
      layoutSections: createExecutionLayoutSections(),
      views: createExecutionViews(),
    },
    {
      name: "kSQL-FlowNet 監査履歴",
      appIdEnvironmentVariable: "KSQL_FLOWNET_AUDIT_APP_ID",
      tokenEnvironmentVariable: "KSQL_FLOWNET_AUDIT_API_TOKEN",
  ...
  function layoutField(fields, code) {
    const field = { type: fields[code].type, code };
    const size = FIELD_SIZES[code];
    return size ? { ...field, size } : field;
  }
  ...
  function createLayout(fields, sections, currentLayout) {
    ...
    sections.forEach(({ label, rows }, sectionIndex) => {
      ...
      for (const codes of rows) {
        layout.push({
          type: "ROW",
          fields: codes.map((code) => layoutField(fields, code)),
        });
      }
    });
  ...
  function createExecutionFields() {
    // 2026-08-30実機スパイクの「Run/State/Lockの許可値union」。個別レコードでの
  ...
  function createAuditFields() {
    // 2026-08-30実機スパイクのInvocation/Attempt許可値union。record type別の検証はアプリ外で行う。
    ...
    return toProperties([
      ...
      number("state_revision_before", "State Revision Before"),
  ...
  function createAuditLayoutSections() {
    return [
      ...
      section("Node Attempt", [
        ...
        ["attempt_key"],
        ["node_id", "job_id"],
        ["attempt_no", "state_revision_before"],
  ...
  try {
    ...
    for (const app of createdApps) {
      console.log(
        `- ${app.name}: アプリID ${app.app} を ${app.appIdEnvironmentVariable} に設定し、レコード追加・閲覧・編集権限のAPIトークンを手動生成して ${app.tokenEnvironmentVariable} に設定する。`,
      );
    }
    console.log(
