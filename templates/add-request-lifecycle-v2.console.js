 ...
 *
 * 注意: APIトークン経由の書込がフィールドアクセス権の影響を受けるかは
 * kintone仕様として未確認。ACL適用後に必ずポーラーの
 * `poll-requests --check` と1件の要求処理で書込可能なことを確認する。
 * 書けなければ本ステップを取り消す(`READ`→権限削除)。
 */
(async () => {
  "use strict";
  ...
  try {
    ...
    console.warn(
      "APIトークン経由の書込への影響はkintone仕様として未確認です。ACL適用後に必ずポーラーの poll-requests --check と1件の要求処理で書込可能なことを確認してください。書けなければ本ステップを取り消してください(READ→権限削除)。",
    );
    step = "ACL適用確認";
    if (
      !confirm(
