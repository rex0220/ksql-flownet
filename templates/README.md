...
本ディレクトリのConsoleスクリプト群は、既存アプリへの追補と開発環境の構築に使用します。既存の実行管理アプリへ3種の関連レコード一覧を追加する場合は、`add-run-related-lists.console.js`へ実行管理・監査履歴・操作要求・JOBログの4アプリIDを入力してください。既に存在する関連レコードフィールドは変更せず、未追加のものだけを追加します。既存の操作要求アプリへCLOSE・処理前取消・取消済み一覧・フィールドACLを追補する場合は、`add-request-lifecycle-v2.console.js`を使用してください。
...
[`icons/`](./icons/)に4アプリ分のアイコン(PNG 256×256)を置いています。各アプリの**アプリの設定 → 設定 → アイコン**で読み込んでください。アイコンはアプリテンプレートに含まれるため、配布テンプレートに設定しておけばインポート先にも引き継がれます。再生成は`pwsh templates/icons/generate-icons.ps1`です。
