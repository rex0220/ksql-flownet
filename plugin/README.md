...
2. **操作要求アプリID**: START/RERUN/STOP/RELEASE/CLOSEの起票、重複ガード、pending表示と取消に使います。
...
一次対応者には、実行管理・監査履歴・検出または設定したJOBログアプリの閲覧権限と、操作要求アプリの閲覧・追加・編集権限が必要です(編集は「取消」の`cancel_requested`更新に必要で、フィールドアクセス権により他のフィールドは編集できません)。runtimeでは自動検出用のフォームフィールドGET、実行管理・監査履歴・JOBログ・操作要求アプリのレコードGET、操作要求アプリの単票POST、および`cancel_requested`だけを更新する単票PUTを使用します。APIトークン、cursor、Bulk、DELETE、その他のPUTは使用しません。フォームフィールドGETが権限またはAPIエラーで失敗した場合は、保存済み設定だけで動作します。

処理待ち要求は要求単位で表示します。`REQUESTED`かつ作成者本人の要求だけ「取消」を選べます。取消は元に戻せず、再実行には新しい要求の起票が必要です。FAILED/CANCELLED Runに停止holdが残っている場合は先に「解除要求」を出し、解除後に「リラン要求」を出してください。holdのないFAILED/CANCELLED Runでは「リラン要求」と「クローズ要求」を選べます。クローズ後のRunは再開できないため、再集計は補正キーで新規実行します。
