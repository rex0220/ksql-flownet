...
4. **STARTを許可するネットワーク**: 任意。VPSのallowlistで`app_start: true`を付けたネットワークを、`ネットワーク名, network_id`（例: `月次案件集計, monthly_deal_summary`）のCSV形式で1行ずつ登録します。旧形式の`network_id`だけの行も使用できます。二重管理ですが、起動可否の正はサーバー側です。一覧にない値も「その他(自由入力)」から起票でき、許可外の値はポーラーが拒否します。
