...
async function runWorker() {
  ...
  if (finish?.type === "release" && result.success) {
    try {
      ...
      process.send({ type: "released", workerId, success: true, apiCalls: 1 });
    } catch (error) {
      process.send({
        type: "released",
        workerId,
...
function nextMessage(worker, type) {
  return new Promise((resolve, reject) => {
    ...
    const onExit = (code) => {
      if (code !== 0)
        onError(new Error(`workerが異常終了しました (exit ${code})。`));
    };
    const cleanup = () => {
...
export async function runContention({ workers = 2, iterations = 10 } = {}) {
  ...
  for (let iteration = 1; iteration <= iterations; iteration += 1) {
    ...
    const children = Array.from({ length: workers }, (_, index) =>
      fork(fileURLToPath(import.meta.url), ["--worker", `worker-${index + 1}`]),
    );
    const resultPromises = children.map((child) =>
      nextMessage(child, "result"),
    );
    await releaseWhenReady(children, { type: "start", key });
...
if (process.argv[2] === "--worker") {
  runWorker().catch((error) => {
    process.send?.({ type: "fatal", error: summarizeError(error) });
    process.exitCode = 1;
  });
