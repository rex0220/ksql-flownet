...
| P2-08 | 案A v1: 導出プラグイン — **M3完了・M4完了(本番適用手順待ち)** | kintoneプラグインとして実行管理アプリへactivity 4値をread-only表示。導出規則は凍結仕様§7.4と共有test vector(`tests/fixtures/status-activity/`)を単一正本とし、M3実機受入([証跡](./test-results/p2-08-20260901/README.md))とM4の配布・導入・一次対応文書を完了した。本番アプリへの適用とread-only smokeは未実施のため、**npm公開ゲートは未解除** | [仕様](./p2-08-activity-plugin-spec.md)・[実装計画](./p2-08-implementation-plan.md)。PRE-03(案A v0)の後継 |
