...
| P2-09 | ボードからの操作要求起票 — **M0〜M4完了(本番適用待ち)** | [仕様](./p2-09-board-request-spec.md)・[実装計画](./p2-09-implementation-plan.md)どおりM0〜M4完了。ボードからRERUN/STOP/RELEASEを操作要求アプリへ単票起票し、要対応(終端)との2セクション、pendingバッジ、UNKNOWNの連絡導線を実装。M3実機受入合格([証跡](./test-results/p2-09-20260901/README.md))、運用・復旧・配布文書を改訂済み。本番zip更新、操作要求アプリ設定、一次対応者への表示変更周知は適用作業待ち | 2026-09-01 M3実機受入・M4文書/本番適用準備完了。実行管理・監査はGET限定、書込は操作要求アプリへの単票POSTのみ |
