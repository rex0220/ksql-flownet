...
| P2-11 | 不定期ジョブのアプリ起動(START要求) | [仕様DRAFT](./p2-11-adhoc-start-spec.md)。操作要求アプリへSTART種別を追加し、ポーラーがrun-networkを新規起動(allowlist限定・冪等networkのみ・重複はensure-run裁定・FlowNet本体無改修)。第2段でボードへ新規実行ボタン。P2-08計画§9の非対象(correction Runのアプリ化)を明示撤回 | 2026-09-01ユーザー要望(定期cron以外のオンデマンド起動の画面入口)。着手待ち |
