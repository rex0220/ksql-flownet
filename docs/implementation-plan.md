...
| PRE-05 | 候補業務の非冪等棚卸し | 候補ネットワークの全ノードSQLを7項目基準(正本: kintone-ops-roadmap-discussion.md §11.2)で目視分類し、既存のidempotent宣言の正しさ自体を検証する(宣言誤りは自動リランの二重書込に直結)。見落とされやすいのは履歴追記のbare INSERTと集計の自己参照。キー指定DELETEは保守側分類とし個別緩和は実施時判断。結果はP2-05(自動分類)の入力仕様を兼ねる | **Dq-4のクリティカルパス**(討論§11)。全ノード冪等の業務が見つかれば承認者問題が初回導入の前提から外れる。PRE-01〜04と並行可 |
...
| ID | 作業単位 | 概要 | 経緯 |
| --- | --- | --- | --- |
| P2-01 | アプリ起点リラン | kSQL-Flowの`rerun_request`と同型の外付けポーラー方式。実行管理アプリへ要求フィールド(要求フラグ・対象node_id・要求者・要求時刻)を追加し、常駐ポーラーが検出→`run-network --resume-run [--rerun-from]`起動→フラグクリア。FlowNet本体は変更せず、D-30の責務境界(常駐はFlowNet外)とD-13/D-18の監査規律(非冪等拒否はCLI契約が担保)を維持する | 2026-08-31ユーザー要望。Phase 1はCLI起点のみで凍結(常駐ポーリング非所有=D-30、復旧系の認証・証拠強制=D-13/D-18のため) |
| P2-02 | ノード境界の停止要求 | 停止要求フラグをノード境界でのみ検査し、実行中subprocessは完走を待ち、次ノードを起動せずに正常終端する。通常停止がUNKNOWN・resolve-nodeを経由しなくなる。優先度高 | 2026-08-31外部評価R2-3。現状の停止はkill→drain→孤児裁定の最高コスト経路のみ |
