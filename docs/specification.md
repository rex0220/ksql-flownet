...
- 人の操作は操作要求アプリへの新規レコード追加として受ける。既存要求の編集は claim 前取消の `cancel_requested` 更新だけであり、編集して再要求しない。取消にはアプリのレコード編集権限が必要で、編集できるフィールドはフィールドアクセス権で `cancel_requested`(作成者)に限定する(§3.4)。
