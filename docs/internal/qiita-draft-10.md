...
- 連載 #10(最終回)(#1: https://qiita.com/rex0220/items/24470d6223c1b4ed4031、#2: https://qiita.com/rex0220/items/2308e4ccf5a363680d31、#3: https://qiita.com/rex0220/items/45f04c2748570953629b、#4: https://qiita.com/rex0220/items/45a086c83cb1dd992aeb、#5〜#9: 未公開)
...
- 外部レビューの裁定表。採用 231 件・不採用 15 件を、正本と実装で真偽を確認して決めた
...
| kintone 操作・秘密情報 | 人 | アプリ作成、テンプレート登録、プラグイン読込、API トークン発行と転記、npm publish、Qiita 投稿 |
...
| 仕様の外部レビュー | Gemini・ChatGPT・Codex | 凍結前の仕様と公開文書を批判的に読む。裁定は Claude Code、採否表は人が確認する |
| 隣のリポジトリ | 別セッションの Claude Code | kSQL-Flow 側の変更(Execution Contract、CSV 取込)は kSQL-Flow のリポジトリで別セッションが担当。依頼と返信は日付付きの文書で往復 |

ポイントは **実装者とレビュー担当を別の AI にした** ことです。同じモデルが書いて同じモデルが見ると、同じ思い込みを通します。Codex が書いた実装を Claude Code が正本(仕様書・決定記録・Execution Contract)と突き合わせ、逆に Claude Code が起案した仕様を Codex が実装ファイルの行番号付きで批判する、という交差にしました。
...
Gemini・ChatGPT のレビューは人がチャットに貼り、Codex のレビューは `codex exec` で走らせて `docs/internal/*-review-*.md` に出します。Claude Code は裁定者です。指摘ごとに `src/`・`tests/`・仕様書で真偽を確認し、採否表を残し、採用分だけを 1 コミットで反映します。
...
**トークン値を AI に見せない。** クリーンインストール検証で、テスト用アプリのトークンを含むファイルを AI が読み、値がツール出力に表示されました。本番のトークンは含みませんでしたが、検証用トークンの再発行を推奨する記録を残し、`.env-*` を `.gitignore` に追加しました。導入手順書の Claude Code 併用版は、この前提で書いています。AI が作る環境ファイルは値が空の雛形で、値の転記は人が SSH でエディタを開いて行う。ファイルを読むときは変数名だけを報告する。
...
- 実装者とレビュー担当を別の AI にし、正本(仕様・決定記録・契約)で照合する。同じモデルの思い込みを通さない
- 仕様と実装が食い違ったら、実装に合わせず決定記録で再審議する。凍結後の変更は記録を通す
- 外部レビューは裁定表で受ける。採用 231・不採用 15。不採用には正本と実装の根拠を書く
