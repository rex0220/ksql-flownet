...
| P2-16 | 操作要求ライフサイクル v2(P2-10+P2-12+P2-15 統合) | [仕様DRAFT v1](./p2-16-request-lifecycle-v2-spec.md)。CLOSE 種別+`archive-run` CLI(ARCHIVED 書込経路)、`cancel_requested` による claim 前取消(`CANCELLED` 終端・revision 競合裁定)、RELEASE 受理条件を hold の有無へ変更(終端 Run でもアプリから解除可)、status JSON `hold`、ボードの取消/解除/クローズ導線、`cancel_requested` のみの単票 PUT 例外。同じ状態機械・同じ 3 層の改訂のため 1 仕様・1 レビューサイクル | 2026-09-05 ユーザー指示(backlog 推奨順 1 を Go)。v1.0.0 リリース後の次版。M0=外部レビュー→FROZEN、実装は R2 凍結解除後 |
