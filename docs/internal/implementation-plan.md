...
| P2-09 | ボードからの操作要求起票 — **完了(2026-09-01)** | [仕様](./p2-09-board-request-spec.md)・[実装計画](./p2-09-implementation-plan.md)どおりM0〜M4完了。ボードからRERUN/STOP/RELEASEを操作要求アプリへ単票起票し、要対応(終端)との2セクション、pendingバッジ、UNKNOWNの連絡導線を実装。M3実機受入合格([証跡](./test-results/p2-09-20260901/README.md))、運用・復旧・配布文書を改訂済み。**本番適用済み(同日)** — zip v5更新・本番操作要求アプリ設定・ボード表示確認まで完了 | 2026-09-01 M3実機受入・M4文書/本番適用準備完了。実行管理・監査はGET限定、書込は操作要求アプリへの単票POSTのみ |
| P2-10 | 終端Runのクローズ手段 — **P2-16へ統合(2026-09-05)** | 操作要求アプリへ`CLOSE`種別を追加し、ポーラー経由の新CLI`archive-run`が`lifecycle_status = ARCHIVED`を書く(意味論は定義済み — ensure-runがresume拒否)。人が書くのは要求アプリのみ・実行管理を書くのは機械、の規律を維持。**状態を書く新CLI経路のため個別レビュー必須** | 2026-09-01 P2-09外部レビュー第2巡指摘1(リランせず決着した終端Runが要対応リストに永久滞留 — ARCHIVED書込経路が実装に不存在)。P2-09の`lifecycle_status = ACTIVE`フィルタは本タスク完了で実効化 |
| P2-11 | 不定期ジョブのアプリ起動(START要求) — **完了・本番適用済み(2026-09-04、[記録](./test-results/r0-p2-11-production-20260904/README.md))** | [仕様](./p2-11-adhoc-start-spec.md)。操作要求アプリへSTART種別を追加し、ポーラーがrun-networkを新規起動(allowlist限定・冪等networkのみ・重複はensure-run裁定・FlowNet本体無改修)。第2段でボードへ新規実行ボタン。P2-01計画§9の非対象(correction Runのアプリ化)を明示撤回(D-1反映済み) | 2026-09-01ユーザー要望(定期cron以外のオンデマンド起動の画面入口)。仕様FROZEN v6(6巡+M0コード確認4点)。M0〜M4実装完了・スパイク実機受入合格(2026-09-02、[記録](./test-results/p2-11-20260902/README.md))。本番適用待ち |
| P2-12 | 処理前取消(cancel_requested) — **P2-16へ統合(2026-09-05)** | 起票後〜claim前の要求を取り消す: `cancel_requested`チェックボックス+claim時検知で`CANCELLED`終端(ACCEPTED以降は無効、Run化後はSTOPの役割)、ボードpending表示の取消ボタン(確認ダイアログ→取消欄のみのPUT)。**P2-01(REVIEWED)の状態機械・権限モデル・G-09を改訂するため個別レビュー必須**。設計論点(P2-11第6巡から引き継ぎ): revision競合裁定I-04(取消PUTとclaimの楽観ロック、claim成立後に取消成功を表示しない)/ACCEPTED行に取消ボタンを出さない/取消エンジン(M3相当)と取消UI(M4相当)の受入分割/kintoneフィールドアクセス権で`cancel_requested`以外を人に編集不可(運用規律に依存しない)/取消者の証跡はkintone変更履歴を正とする(更新者欄はポーラー書き戻しで上書きされる)/409競合時はUIで「すでに処理開始済み」表示+再読込 | 2026-09-02 P2-11第5巡(ユーザー指摘: 取消手段)で設計、第6巡X-1(Claude)採用で切り出し。全request_type共通の汎用改善 |
| P2-13 | START許可一覧のVPS API配信 | プラグインのSTART許可ネットワーク一覧(現状はプラグイン設定の二重管理 — P2-11案A)を、VPS上の読み取り専用HTTPS APIから取得して一本化する。設計論点: 常設受信サービスの新設(現在SSH以外閉)、ドメイン+Let's Encrypt証明書と自動更新、kintone.plugin.app.proxyでの認証(APIキーはproxy設定に保存)、API停止時は自由入力へfail-open(起動可否の正はポーラー — I-03不変)、レート制限。**Control Planeへの入口追加のため個別仕様+レビュー必須** | 2026-09-02 P2-11実機受入でのユーザー要望(許可一覧からの選択)。開放ネットワークが増えて二重管理が痛くなった時点で仕様化 |
| P2-14 | kintone添付経由のCSV取込アダプタ | 一次対応者がSSHなしで入力CSVを渡せるようにする: kintoneのファイル提供アプリへ添付→ポーラー系cronが添付をDLして`KSQL_FLOWNET_IO_DIR/in/`の規約パスへ配置(sha256記録・冪等)→既存のinputs取込へ接続。出力側も同様に`out/`の完成ファイルを添付として提供アプリへ返す配信アダプタを検討。csv-io-implementation-plan B-2で「スコープ外の別作業(小)」と裁定済み。設計論点: 配置の冪等性(同名再添付)、baseline/MUTATEDとの整合、添付サイズ上限、提供アプリのACL。**FlowNet本体のIO封じ込め仕様には触れない(外側のアダプタ)** | 2026-09-04ユーザー要望(CSVをVPSへ置く/取り出す構成・手順)。当面の正は[docs/csv-io-operations.md](../csv-io-operations.md)のSSH(SCP/SFTP)手順 |
| P2-17 | 導入手順書の本番硬化(実行専用ユーザー分離・cron の PATH と flock・logrotate) | v1.0.0 の導入手順書は root で SSH・導入・cron 実行する構成。本番向けに、SSH 用の管理ユーザーとジョブ実行専用ユーザーの分離(環境ファイル root:ksql 0640、`/etc/ksql-flownet/`、`/var/lib/ksql-flownet/`)、crontab の `PATH` 明示と `flock -n`、logrotate 設定例を手順書 §6〜§10 へ追補する。CLI の変更は不要 | 2026-09-06 Qiita #2 導入編の外部レビュー(ChatGPT)。記事側は注記と cron 例で対応済み([裁定](./qiita-review-2-20260906.md)) |
| P2-15 | 終端Runに残ったholdのアプリ解除 — **P2-16へ統合(2026-09-05)** | STOP要求後に実行中SQLが失敗しRunがFAILED/CANCELLEDになると、CANCEL_REQUESTのholdが残るがactivityが付かない(run-activity.tsは終端でnull)ため、ボードはリラン要求を出し、RERUNはensure-runの`RUN_ON_HOLD`、アプリRELEASEはポーラーの`RUN_NOT_ON_HOLD`で拒否される。現状はCLI `cancel-run --release`のみ。改善案: (a) 終端Runでもholdがあれば解除要求ボタンを優先表示し、ポーラーのRELEASE受理条件を「activity STOPPED」から「CANCEL_REQUESTがREQUESTED/ACCEPTED」へ変更、または(b) ノード失敗で終端した時点でholdをRELEASED相当に落とす。状態機械とG-09の改訂を伴うためP2-12と合わせて個別レビュー | 2026-09-05 ChatGPT仕様書再評価②で指摘。実装確認済み。仕様書§7.3/§9・runbookに制約として明記済み。R2凍結中のため次版へ |
| P2-16 | 操作要求ライフサイクル v2(P2-10+P2-12+P2-15 統合) — **完了・本番適用済み(2026-09-05、[記録](./test-results/p2-16-production-20260905/README.md))** | [仕様](./p2-16-request-lifecycle-v2-spec.md)(FROZEN v1)。CLOSE 種別+`archive-run` CLI(ARCHIVED 書込経路)、`cancel_requested` による claim 前取消(`CANCELLED` 終端・revision 競合裁定)、RELEASE 受理条件を hold の有無へ変更(終端 Run でもアプリから解除可)、status JSON `hold`、ボードの取消/解除/クローズ導線、`cancel_requested` のみの単票 PUT 例外。M0〜M4は実装・実機受入済みで、M5は統合仕様・P2-01・一次対応・復旧runbook・READMEの改訂 | 2026-09-05 M0〜M4完了。実機受入は`test-results/p2-16-m3-20260905/`・`p2-16-m4-20260905/` |
