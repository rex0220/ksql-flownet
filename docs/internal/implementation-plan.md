...
| P2-12 | 処理前取消(cancel_requested) | 起票後〜claim前の要求を取り消す: `cancel_requested`チェックボックス+claim時検知で`CANCELLED`終端(ACCEPTED以降は無効、Run化後はSTOPの役割)、ボードpending表示の取消ボタン(確認ダイアログ→取消欄のみのPUT)。**P2-01(REVIEWED)の状態機械・権限モデル・G-09を改訂するため個別レビュー必須**。設計論点(P2-11第6巡から引き継ぎ): revision競合裁定I-04(取消PUTとclaimの楽観ロック、claim成立後に取消成功を表示しない)/ACCEPTED行に取消ボタンを出さない/取消エンジン(M3相当)と取消UI(M4相当)の受入分割/kintoneフィールドアクセス権で`cancel_requested`以外を人に編集不可(運用規律に依存しない)/取消者の証跡はkintone変更履歴を正とする(更新者欄はポーラー書き戻しで上書きされる)/409競合時はUIで「すでに処理開始済み」表示+再読込 | 2026-09-02 P2-11第5巡(ユーザー指摘: 取消手段)で設計、第6巡X-1(Claude)採用で切り出し。全request_type共通の汎用改善 |
