...
| P2-16 | 操作要求ライフサイクル v2(P2-10+P2-12+P2-15 統合) — **M0〜M4 完了・M5 文書改訂中(2026-09-05)** | [仕様](./p2-16-request-lifecycle-v2-spec.md)(FROZEN v1)。CLOSE 種別+`archive-run` CLI(ARCHIVED 書込経路)、`cancel_requested` による claim 前取消(`CANCELLED` 終端・revision 競合裁定)、RELEASE 受理条件を hold の有無へ変更(終端 Run でもアプリから解除可)、status JSON `hold`、ボードの取消/解除/クローズ導線、`cancel_requested` のみの単票 PUT 例外。M0〜M4は実装・実機受入済みで、M5は統合仕様・P2-01・一次対応・復旧runbook・READMEの改訂 | 2026-09-05 M0〜M4完了。実機受入は`test-results/p2-16-m3-20260905/`・`p2-16-m4-20260905/` |
