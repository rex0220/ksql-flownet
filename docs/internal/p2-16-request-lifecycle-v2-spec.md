...
| `cancel_requested` | **新設・チェックボックス(値 `取消`)**。人が編集できる唯一の機械管理外フィールドで、フィールドアクセス権は「作成者=編集可・everyone=閲覧のみ」(U-4)。`REQUESTED` の間だけ意味を持つ。**不可逆**: 一度立てた取消の解除(チェック外し)は仕様上不可とする。ボードは解除操作を提供せず、kintone 画面での直接解除は運用違反(変更履歴に残る)。ポーラーは取得時点の値で裁定するため、解除されていれば claim されるが、これは保証された復活手段ではない。復活したい場合は新規起票する |
...
| U-4 | 取消ボタンの表示権限 | **起票者本人のみ**にボードが表示し、API 側は kintone のフィールド権限で二重に弾く。**実機確認 2026-09-05: フィールドアクセス権の対象に「作成者」を指定できる**ため、`cancel_requested` は「作成者=編集可(`FIELD_ENTITY: 作成者`、上位)・everyone=閲覧のみ」で確定。追補スクリプトはこの構成を適用する。他人による直接 PUT の拒否を受入 9 で確認する。運用グループへの拡張は必要になった時点で |
