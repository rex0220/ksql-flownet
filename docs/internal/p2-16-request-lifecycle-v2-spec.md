...
- 状態: **DRAFT v1**(2026-09-05 起案・外部レビュー前)
...
| `cancel_requested` | **新設・チェックボックス(値 `取消`)**。人が編集できる唯一の機械管理外フィールド。`REQUESTED` の間だけ意味を持つ |
| フィールドアクセス権 | `request_type` / `run_id` / `network_id` / `business_key` / `scheduled_for` / `rerun_from_node` / `reason` は**起票後は編集不可**、`cancel_requested` のみ起票者(または運用グループ)が編集可、機械フィールド 6 種は閲覧のみ(X-4 引継ぎ: 運用規律ではなく kintone の権限で担保) |
| 一覧 | `01_未処理要求` に `cancel_requested` 列を追加。`03_取消済み`(request_state in CANCELLED)を追加するかは任意 |
...
- pending リンク先(START要求セクション・進行中 Run の pending バッジ)に、`request_state = REQUESTED` の行だけ **「取消」ボタン**を出す。`ACCEPTED` の行には出さない(X-3)
- 押下 → 確認ダイアログ(要求種別・対象・理由を再表示)→ `cancel_requested` のみを含む単票 PUT(`$revision` 付き)
...
hold の有無はプラグインが既に読んでいる `CANCEL_REQUEST` レコードから導出する(CLI status を待たない)。
...
| --- | --- | --- | --- |
| `CLOSE` | Run が `FAILED` / `CANCELLED`、`lifecycle_status = ACTIVE`、live owner なし、**hold なし**(hold があれば先に RELEASE)。`SUCCESS` は対象外(`RUN_STATUS_NOT_CLOSABLE`)、`UNKNOWN` は `resolve-node` で解決してから(`RUN_UNKNOWN_NOT_CLOSABLE`) | 新 CLI `archive-run --run-id <run_id> --reason-file <一時ファイル>` を子プロセス起動 | `RUN_ARCHIVED` |
...
| 9 | フィールドアクセス権: 起票者が `reason` 等を編集できず `cancel_requested` だけ編集できる(テンプレート検証) | 実機・手動 |
...
- **U-1** `03_取消済み` 一覧を作るか(pending 一覧から消えれば十分か)
- **U-2** CLOSE を SUCCESS Run にも許すか(履歴整理目的)。現案は不可 — SUCCESS は要対応一覧に出ないため需要が薄い
- **U-3** `archive-run` を CLI 単体でも二次対応者が直接使えるようにするか(現案: 可。ポーラー経由と同じ検証)
- **U-4** 取消ボタンの表示権限(起票者本人のみか、運用グループか)。kintone のフィールド権限に委ねる案

## 10. 凍結条件

外部レビュー 3 系統で「実装不能」「不変条件違反」「受入の矛盾」の指摘がゼロになり、U-1〜U-4 に決定が付いた時点で FROZEN とする。
