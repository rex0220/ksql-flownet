...
- 状態: **M0 の設計判断を本書で確定。段階1はエンジン `/flow` 公開APIのIMPORT供給ギャップを解消してから着手する**
...
1. 業務データ本体はFlowNetのbundle、stdout、kintone監査アプリを通さない。FlowNetが扱うのはパス、sha256、bytes、rows、encoding等のメタデータだけとする。
...
| X-4 | **段階1は専用の`input_files` / `output_files`監査列を追加しない。** kSQL-FlowのExecution Result JSONを正本とし、監査アプリにはresult codeと秘密を含まない短い要約だけを既存の安全化済みメッセージ規律で残す | アプリtemplateとrecord schema versionは段階1で変更しない。ただしクラッシュ時にも比較元を失わないよう、FlowNetはsubprocess起動前にsha256/bytesを含むbaseline receiptを `KSQL_FLOWNET_IO_DIR/.flownet-meta/runs/<run_id>/inputs/<node_id>.json` へtemp+renameで保存する。完了後はExecution Resultの`input_files`と照合してreceiptを完成させる。これは業務データ本体ではなく、共有・耐久ストレージ上の運用メタデータである。専用監査列は需要と検索要件が確定した将来のschema migrationへ切り出す |
| X-5 | `INPUT_FILE_MISSING`、`INPUT_FILE_MUTATED`、`INPUT_RETENTION_EXPIRED`を区別する。保持期間は既定30日、環境変数で明示変更可とし、Run作成日時から保持期限を過ぎたRunはresume/`--rerun-from`しない | 期限内の不在はFAILEDとして再試行可能で、同一failure kindが3回連続すれば既存`RETRY_BRAKE`。baselineと異なるsha256はMUTATEDとしてfail-closed。期限超過はファイル状態を調べず拒否し、補正business keyで新Runを作る。運用が30日を超える再開を必要とする場合は、先に保持期間を延ばす |
| X-6 | rootはorchestrator環境変数 `KSQL_FLOWNET_IO_DIR` で指定し、入力保持日数は `KSQL_FLOWNET_IO_RETENTION_DAYS`（既定30）とする | rootは起動時に絶対path、既存directory、共有・耐久領域であることを検証する。network定義とSQLには絶対pathを持たせない。入力は`in/`、出力は`out/`、receiptは`.flownet-meta/`へ分離する |
...
1. `run`へrepeatableな`--import-csv <name>=<absolute-path>`を追加する。今回のFlowNet schemaはCSVだけを対象とするが、既存engine契約との対称性を保つためstandalone kSQL-Flowでは`--import-json`も同じloader層へ配線してよい。
...
contract testはCLI parser、capability、Execution Result schema、stdout純度、source loader、IMPORT実行を分ける。最低限、UTF-8/SJIS、repeatable引数、source未供給、同名重複、期待sha256の欠落・不一致、通常ファイル以外、10MiB等のengine上限、`maxRecords`、途中クラッシュ後の再実行を含める。
...
| 種別 | ファイル | 変更内容 |
| --- | --- | --- |
| CLI | `src/cli.ts` | repeatable `--import-csv` / `--expected-import-sha256`、構文・重複検証、orchestrator mode許可 |
...
#### 3.2.4 baseline receipt、resume、`--rerun-from`

入力を持つノードごとに、最初のsubprocess起動前に次のreceiptを排他的に作る。
...
```json
{
  ...
  "files": [
    {
      "name": "sales",
      "displayPath": "in/sales_<encoded-key>_<encoded-profile>.csv",
      "sha256": "<64 hex>",
      "bytes": 1234
    }
  ]
...
- temp fileを同一directoryへ書き、flush後renameする。既存receiptがあれば上書きしない。
- 競合して作成された場合は既存値を読み直し、同じhashなら続行、異なればMUTATEDとする。
- 通常resumeではFAILED/CANCELLEDからWAITINGへ戻るimportノードだけ、`--rerun-from`では選択集合内のimportノード（元SUCCESSを含む）だけをreceiptと比較する。
...
- kSQL-FlowからExecution Resultを受けたら、`input_files`のname/sha256/bytesがpreflight値と完全一致することも検証する。不一致は`INVALID_EXECUTION_RESULT`としてUNKNOWNへ倒す。rows/encodingをreceiptへ追記する。
...
- Run作成から保持日数を超えたresume/`--rerun-from`は、ファイルの有無にかかわらず `INPUT_RETENTION_EXPIRED` でInvocation作成前に拒否する。新しい補正business keyを案内する。

途中クラッシュ時はExecution Resultが無くてもpre-execution receiptが残る。同じCSVでresumeすれば`ON DUPLICATE` upsertが同じキーへ収束し、差替えCSVならsubprocess前にMUTATEDで止まる。
...
| 新規IO helper | `src/io/io-config.ts`, `src/io/io-path.ts`, `src/io/input-receipt.ts` | env、percent encoding、Windows/Posix containment、symlink/junction、realpath、regular file、atomic receipt、競合 |
...
| result | `src/executor/result-classifier.ts`, `src/executor/attempt-executor.ts` | `input_files` shape・preflight一致、MISSING/MUTATED/INVALIDの終端、SQL未開始 |
| production wiring | `src/cli/run-network-command.ts` | 2環境変数、CSV未使用時の互換、共有root注入 |
| fixtures/tests | `tests/fixtures/execution-result/*`, `tests/unit/run-subprocess.test.mjs`, `attempt-executor.test.mjs`, `sequential-scheduler.test.mjs`, `ensure-run.test.mjs`, `execution-result-schema.test.mjs` | argv、result、receipt、resume/rerun、秘密・業務値非出力 |
| docs | `docs/execution-contract-v1.md`, `docs/internal/job-network-phase1-spec.md`, `docs/specification.md`, `docs/runbook-recovery.md`, `docs/ops-first-response.md`, `docs/internal/job-network-examples.md` | Contract v1.1、schema、保持期限、一次対応、冪等IMPORT例 |
...
FlowNetは `<KSQL_FLOWNET_IO_DIR>/out`配下へ同じallowlist規則で解決し、`--export-csv`を渡す。kSQL-FlowのExecution Result `output_files`を検証してreceiptへ保存する。段階1のX-4決定と同じく専用kintone列は追加せず、result JSON正本と安全な監査要約に留める。
...
変更対象は段階1の実行経路に加え、engineのserializer / sink resolver、kSQL-Flowの`src/cli.ts`・`src/commands/run.ts`・結果schema、FlowNetのnetwork schema・IO helper・scheduler・subprocess・result classifier・receipt、各contract test、言語リファレンスである。
...
| 8 | 差替え後resumeはMUTATED、成功済み非対象nodeは読まない | 1 | kSQL-FlowNet | receiptとsubprocess呼出し有無をunit/E2E |
| 9 | アダプタが元pathへ保持し、失敗後resumeで読める | 1 | 運用adapter + kSQL-FlowNet | 期限内E2E |
| 10 | markerなしは先頭ASSERTで止まりimport Attemptなし | 1 | network SQL + kSQL-FlowNet | scheduler/E2E。入力の一律pre-flightがないことも確認 |
| 11 | `ON ERROR SKIP INTO #err` + `ASSERT`、件数が監査へ載る | 1 | engine + kSQL-Flow | Execution Resultの安全な件数metadataとE2E |
| 12 | allowlist外、symlink/junction、traversal拒否 | 1 | kSQL-FlowNet | Windows/Posix unit + E2E、subprocess未起動 |
| 13 | `input_files`記録、result/監査に業務値なし | 1 | 両リポジトリ | schema/secret test + 実network |
...
| 17 | `output_files`記録、同一Run再実行で同一sha256 | 2 | 両リポジトリ | result/receipt + `--rerun-from` E2E |
...
受入18は段階1完了条件であり、単なる「再実行が成功した」では合格にしない。クラッシュ直前までの部分適用を意図的に作り、対象アプリを重複禁止keyで集計して全keyの件数が1であることを確認する。

## 6. 見積りと順序制約

見積りは1人日を実装・review修正・自動testまでとし、実機環境の待ち時間とリリース承認は含めない。X-4のreceipt/result JSON保持と照合を明示的に加算した。

| 区分 | 作業 | 見積り |
| --- | --- | ---: |
| M0 | C-1/C-2証跡固定、Contract/API設計、X-1〜X-6確定 | 1〜2人日 |
| engine前提 | `/flow` named import source公開API、型、test、publish | 2〜4人日 |
...
| 段階1 FlowNet監査/X-4 | baseline receipt、atomic保存、result JSON索引、hash照合、resume/rerun | **4〜7人日** |
...
| 段階2 FlowNet | outputs schema、path、argv、receipt、`output_files` | 3〜5人日 |
...
M0確定
  ...
  → FlowNet inputs・security・receipt・resume
  → 段階1受入2, 5〜13, 18, 19
  → 1万件実測（受入4、サブテーブル有無）
  → gate review
  → 段階2(b)
...
| X-4 | §1-4、§4、§8 | 専用監査列は追加せずExecution Result JSONを正本とする決定、pre-execution baseline receipt、監査の安全な要約、将来のschema migration切出しを追記。見積りを小から本書の4〜7人日相当へ修正 |
| X-5 | §1-4、§1-5、§7、受入 | `INPUT_FILE_MISSING` / `INPUT_FILE_MUTATED` / `INPUT_RETENTION_EXPIRED`を分離。既定30日、期限後はresumeせず補正keyの新Runとする。MISSING 3連続だけが通常のRETRY_BRAKE経路になることを明記 |
| X-6 | §1-2、§1-3、§4、§7 | `KSQL_FLOWNET_IO_DIR`、`in/`・`out/`・`.flownet-meta/`、networkは相対patternのみを追記。`KSQL_FLOWNET_IO_RETENTION_DAYS`も運用設定として追加 |
...
段階1の実装着手判定は、engine `/flow` APIの公開versionとContract v1.1のsource/error/result schemaがreview済みであること。段階1の完了判定は受入2、4〜13、18、19が合格し、1万件実測値が文書化されていること。段階2の完了判定は受入1、3、14〜17が合格し、同一Run再実行のoutput sha256一致と配信marker境界が実機で確認されていることとする。
