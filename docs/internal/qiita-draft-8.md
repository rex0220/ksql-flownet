...
Network 単位の排他を保証する主役は Network ロックです。ロックの作成・更新は重複禁止 INSERT または `revision` 付き更新と owner の再確認で行い、取得の競合を一方だけの成功に収束させます。`record_key` は同じ業務キーの Run の重複を、kSQL-Flow のジョブロックは同じジョブの重複実行を止める追加の防御で、それぞれ保護する範囲が違います(Network ロックの代替ではありません)。ロックの確認から更新までの隙間(TOCTOU)は残余リスクとして残しており、その窓を抜けた場合に業務キーとジョブの単位で被害を止めるのがこの 2 層です。
