...
補正モードでは、定期キーを自動導出せず指定した業務キーを使います。ただし SQL の基準時刻を確定するため対象期間も必須です(業務キーだけを渡すと、基準時刻が起動時刻になって「8 月の補正キーで 9 月の断面を集計する」静かな誤りが起きるため、設計時に両方必須へ改めました)。この組合せは実機で確認済みで、筆者の本番でも補正 START はこの形で動いています。
