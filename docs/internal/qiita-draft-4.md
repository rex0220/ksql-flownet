...
全コードは[統合仕様書 §6.7](https://github.com/rex0220/ksql-flownet/blob/v1.0.0/docs/specification.md)にあります。共通のルールは 2 つです。同じ依頼をそのまま繰り返さない(`OK` なら Run の状態を確認し、それ以外は結果コードに応じて対応する)。`STALE` と UNKNOWN は照合が終わるまで何もしない。
