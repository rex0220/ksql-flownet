...
- プラグインv5(スパイク受入と同一zip)を本番へ更新し、本番実行管理アプリのプラグイン設定へ操作要求アプリを追加指定(監査設定は保持)。「00_Run状況」ボードの2セクション表示を確認
- これにより一次対応は「ボードで発見→ボードから起票(RERUN/STOP/RELEASE)→処理待ちバッジ/要求レコードで追跡」まで画面で完結。処理は稼働中の本番ポーラー(5分cron)が担当
