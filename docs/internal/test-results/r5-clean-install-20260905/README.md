...
- 取消 smoke の起票者は `rex0220`。一次対応者相当(手順書 §4 の権限のみ、cybozu 管理者でない)のアカウントかどうかをユーザーに確認中。管理者アカウントだった場合は別アカウントで起票・取消を再実施する
- 検証用の cron 2 行は R5 完了時に削除する(`/root/crontab.bak-r5-*` に登録前の状態を保存済み)。`/opt/ksql-r5` と環境ファイルは検証後に削除してよい
- 検証用 4 アプリのトークンは、値が Claude のツール出力に表示された経緯があるため R5 完了後に再発行を推奨(本番トークンは含まない)
