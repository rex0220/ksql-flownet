...
- [ ] 取消 smoke を**一次対応者相当のアカウント**(手順書 §4 の表どおり: 操作要求は閲覧・追加・編集+アプリ管理、実行管理・監査履歴は閲覧+アプリ管理。cybozu 管理者ではない)で実施し、`cancel_requested` の PUT が通ること・他フィールドは編集できないことを確認(ChatGPT 導入手順書レビュー指摘1。本番 smoke は管理者アカウントだったため未検証。2026-09-05 ユーザー決定: 運用上は一次対応者=アプリ管理者)
