...
- [ ] 別スペースへ試験インポートし、関連レコードの参照先が張り替わること・プラグインのゼロコンフィグ自動検出が効くことを確認。テンプレートは実行管理アプリのプラグイン参照(plugin id)を含むため、プラグイン未読込のスペースでインポートした場合の挙動(エラーか参照が落ちるか)も確認し、導入手順書のインポート順(プラグイン読込→テンプレート)へ反映
